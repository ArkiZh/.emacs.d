...
#+begin_src emacs-lisp
  ...
  ;;----------------------------------------------------------------------------
  (defvar arki/graphic-frame-callbacks nil "Store callbacks for graphic environment. Tuple of: func, can repeat?, invoke count so far")
...
#+begin_src emacs-lisp
  ...
  (arki/add-hook-for-graphic-frame-init 'init-graphic-theme)
...
** Org-protocol-capture-html (disabled)
#+begin_src emacs-lisp
  ;; (server-start)
