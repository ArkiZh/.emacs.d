...
#+begin_src emacs-lisp
  ...
  (defun arki/window-move-up-border-up (delta)
    (interactive "P")
    (arki/window-move-border-right "up" (arki/interactive-convert-prefix-to-element delta) t))

  (defun arki/window-move-up-border-down (delta)
    (interactive "P")
    (arki/window-move-border-right "up" (arki/interactive-convert-prefix-to-element delta)))

  (defun arki/window-move-down-border-up (delta)
    (interactive "P")
    (arki/window-move-border-right "down" (arki/interactive-convert-prefix-to-element delta) t))

  (defun arki/window-move-down-border-down (delta)
    (interactive "P")
    (arki/window-move-border-right "down" (arki/interactive-convert-prefix-to-element delta)))
...
#+begin_src emacs-lisp
  ...
  (defcustom arki/desktop-current-desktop nil "The default desktop." :group 'arki/config)
  ...
  (defun arki/desktop-save (&optional desktop-name release only-if-changed)
    (interactive (list
                  (ido-completing-read "Save to this desktop:" (let* ((dirs (directory-files arki/desktop-dir nil "[^.]")))
  ...
  (defun arki/desktop-remove (desktop-name)
    (interactive (list
                  (ido-completing-read "Remove this desktop:" (directory-files arki/desktop-dir nil "[^.]") nil t arki/desktop-current-desktop)))
    (when desktop-name
      (let ((cur-desktop-dir (expand-file-name desktop-name arki/desktop-dir)))
        (when (file-directory-p cur-desktop-dir)
          ...
          (when (string= desktop-name arki/desktop-current-desktop)
            (customize-save-variable 'arki/desktop-current-desktop nil))
          (message "Desktop removed: %s" desktop-name)))))

  (defun arki/desktop-read (&optional desktop-name)
    (interactive (list
                  (ido-completing-read "Read this desktop:" (directory-files arki/desktop-dir nil "[^.]") nil t arki/desktop-current-desktop)))
...
#+begin_src emacs-lisp
  ...
  Press <a> to save desktop.   (Append)
  Press <s> to select desktop. (Select)
  Press <d> to remove desktop. (Delete)
  ...
  (defun arki/layout ()
    ...
    (set-transient-map
     (let ((map (make-sparse-keymap)))
       ...
       (define-key map (kbd "a") 'arki/desktop-save)
       (define-key map (kbd "s") 'arki/desktop-read)
       (define-key map (kbd "d") 'arki/desktop-remove)
...
#+begin_src emacs-lisp
  ...
  (arki/package-archives-mirror-switch "emacs-china")
...
#+begin_src emacs-lisp :tangle no
  ...
  ;; Get tmp buffers and save to desktop location.
  (dolist (v (buffer-list))
    (let* ((cur-name (buffer-name v))
           (cur-file (buffer-file-name v))
           (cur-mode (buffer-local-value 'major-mode v))
           (cur-modified (buffer-modified-p v)))
      (when (and (not (string-match-p "^[ *]" cur-name))
                 (not (equal 'dired-mode cur-mode))
                 (not cur-file))
        (message "%S %S %S %S" cur-modified cur-name cur-mode cur-file)
        (write-file (expand-file-name "scratch" arki/desktop-dir))
