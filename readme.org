...
#+begin_src emacs-lisp :tangle no
  ...
  (defvar arki/window-move-help-message
    "Move window borders by arrows. Default to the right or down border. With <M> for the left or up. With <C-M> for both borders."
    "The help message for window move.")
  (defun arki/window-move ()
    (interactive)
    ...
    (set-transient-map
     (let ((map (make-sparse-keymap)))
       ...
       (define-key map "?" (lambda () (interactive) (message arki/window-move-help-message)))
       map)
     t))

  (arki/define-key "w" 'arki/window-move 'arki/prefix-keymap)
#+end_src
